...
/** Estimate LLM cost from a text string (approximation, ~4 chars/token). */
export function llmCostFromText(providerName, text, isOutput = true) {
  const tokens = Math.ceil(text.length / 4);
  const rates = LLM_USD_PER_1M_TOKENS[providerName];
  if (!rates) return 0;
  const rate = isOutput ? rates.output : rates.input;
  return round((tokens / 1_000_000) * (rate ?? 0));
}
