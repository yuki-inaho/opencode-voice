...
import { llmCost, llmCostFromText } from "./costs.js";
...
export function createLlmClient(options = {}, logger) {
  ...
  // Estimation-only helper for the sidebar (no API call).
  complete.estimate = (text) => llmCostFromText(provider, text, true);
  return complete;
}
