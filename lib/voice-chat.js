...
export function createVoiceChat({ api, client, kv, complete, toast, log, options = {}, stt }) {
  ...
  async function stopAndSend() {
    ...
    try {
      ...
      // Wait for the assistant reply and speak it.
      await speakPendingReply();
    } catch (err) {
  ...
  /** Poll the session for a new assistant message and play it via TTS. */
  async function speakPendingReply(timeoutMs = 60000) {
    ...
    while (Date.now() - started < timeoutMs) {
      ...
      if (last && last.messageID !== lastSpokenMessageID) {
